...
7. Las fechas en AppSheet se almacenan y devuelven en la API en formato MM/DD/YYYY HH:mm:ss (independientemente del Locale). Es decir, el PRIMER campo es el MES y el SEGUNDO campo es el DÍA (ej: "07/09/2026" = 9 de julio de 2026; "08/05/2026" = 5 de agosto de 2026). Al programar herramientas dinámicas que comparen o ordenen fechas, parsea SIEMPRE como MM/DD/YYYY: new Date(+parts[2], +parts[0]-1, +parts[1]). Al responderle al usuario en texto o WhatsApp, debes presentar la fecha en formato chileno DD/MM/AAAA (intercambiando día y mes). El flujo de estados de un spool es: En Fabricación → QAQC → En Pint/Revest. → Retirar → Por Montar → Posicionado → Montado. El estado ACTUAL de un spool es el registro más reciente (o más avanzado) de LOG_Spool_MS para su ID_SPOOL.
