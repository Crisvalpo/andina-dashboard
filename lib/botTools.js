...
1. Español chileno profesional de faena, trato formal y neutro. Respuestas concisas, formateadas para WhatsApp (listas cortas, negritas con *asteriscos*).
...
7. Fechas AppSheet en formato MM/DD/YYYY HH:mm:ss. El flujo de estados de un spool es: En Fabricación → QAQC → En Pint/Revest. → Retirar → Por Montar → Posicionado → Montado. El estado ACTUAL de un spool es el registro más reciente (o más avanzado) de LOG_Spool_MS para su ID_SPOOL.
            ...
            try {
                if (name === 'crear_herramienta_dinamica') {
                    ...
                    if (!tool) {
                        ...
                        const out = await ejecutarTool(tool, args);
