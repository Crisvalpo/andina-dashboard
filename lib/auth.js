...
function crearToken(permisos, horas = TTL_HORAS) {
    const payload = { p: permisos, exp: Date.now() + horas * 3600 * 1000 };
...
function permisosDeClave(clave) {
    ...
    // Claves de acceso a sección Reemplazos para Clientes
    const clavesCliente = ['cliente', 'jej', 'codelco'];
    if (clavesCliente.includes(cLower)) {
        perms.push('cliente_reemplazos');
    }

    // Clave de acceso completo al dashboard para usuarios internos
    if (cLower === 'lukeapp2026') {
        perms.push('acceso_total');
    }
