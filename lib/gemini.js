...
2. VÁLVULA. Identificador: ID tipo "VAL085" — el usuario puede decir "válvula 85", "la VAL085", "válvula ochenta y cinco" → normaliza a "VAL85" o el número solo; el sistema lo resuelve.
   Una válvula pasa por ETAPAS (en orden): ${estadosValvula.join(' → ')}. Si el usuario no dice la etapa, asume "${estadosValvula[estadosValvula.length - 1]}".
   Solo se puede avanzar: si ya está ${estadosValvula[estadosValvula.length - 1]}, no se vuelve atrás. El sistema pre-llena línea y diámetro automáticamente.
  ...
  "items": [
    {
      ...
      "status": "estado (spools: uno de los permitidos; válvula: una de sus etapas ${estadosValvula.join('|')}; soporte: Montado) o null",
