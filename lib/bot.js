...
function formatearFechaChile(fechaStr) {
    ...
    if (match) {
        const [, mm, dd, yyyy, resto] = match;
        return `${dd}/${mm}/${yyyy}${resto}`;
    }
    return fechaStr;
}
...
async function registrarMontajeValvula({ valvula, hoja, usuario, rol }) {
    const fila = {
        'ID_MontajeValvula': crypto.randomBytes(4).toString('hex'),
...
async function registrarMontajeSoporte({ soporte, soldador, responsable, observacion, usuario, rol }) {
    const fila = {
        'ID_MontajeSoporte': crypto.randomBytes(4).toString('hex'),
...
async function handleWhatsappIncoming(req, res) {
    ...
    try {
        ...
        // Compatibilidad: el schema nuevo usa "item"+"entidad"; el viejo usaba "spool"
        const itemDicho = ia.item || ia.spool || null;
        const entidad = ia.entidad || 'spool';
        // Si el modelo no manda el flag, no bloquear el registro
        const listoParaRegistrar = ia.listo_para_registrar !== false;

        if (ia.intencion === 'REGISTRAR_AVANCE' && itemDicho && !listoParaRegistrar) {
            // Conversación guiada: Andi confirma lo capturado y pregunta los
            // campos opcionales UNA vez. La respuesta conversacional viene de Gemini.
            action = 'GUIA_CAMPOS_OPCIONALES';

        } else if (ia.intencion === 'REGISTRAR_AVANCE' && entidad === 'valvula' && itemDicho) {
            const r = await resolverValvula(itemDicho);
            if (!r.encontrado) {
                respuesta = `🔍 No encontré la válvula "${itemDicho}" en el maestro del proyecto (formato VAL001...VAL167). Verifica el número.`;
                action = 'VALVULA_NO_ENCONTRADA';
            } else {
                const previo = await montajeExistente('valvula', r.valvula.id, usuario.rol);
                if (previo) {
                    respuesta = `ℹ️ La válvula *${r.valvula.id}* ya está registrada como *${previo['Status'] || 'Montada'}* el ${formatearFechaChile(previo['fecha'])} por ${previo['usuarioReporte'] || '—'}. No se duplicó el registro.`;
                    action = 'VALVULA_YA_MONTADA';
                } else {
                    ...
                    await guardarRegistro({
                        telefono, spool_tag: r.valvula.id, id_spool: r.valvula.linea,
                        status: 'Montada', observacion: ia.observacion || ia.hoja,
                        appsheet_ok: true,
                        metadata: { entidad: 'valvula', id_montaje: fila['ID_MontajeValvula'] }
                    });
                    respuesta = `✅ *Montaje de válvula registrado*\n\n🔧 Válvula: *${r.valvula.id}*` +
                        `\n📏 Diámetro: ${r.valvula.nps || '—'}` +
                        `\n🧵 Línea: ${r.valvula.linea}` +
                        (ia.hoja ? `\n📄 Hoja: ${ia.hoja}` : '') +
                        `\n👤 ${usuario.nombre}`;
                    action = 'VALVULA_REGISTRADA';
                }
            }

        } else if (ia.intencion === 'REGISTRAR_AVANCE' && entidad === 'soporte' && itemDicho) {
            const r = await resolverSoporte(itemDicho);
            if (!r.encontrado) {
                respuesta = `🔍 No encontré el soporte con ITEM "${itemDicho}" en el maestro del proyecto. Verifica el número.`;
                action = 'SOPORTE_NO_ENCONTRADO';
            } else {
                const previo = await montajeExistente('soporte', r.soporte.id, usuario.rol);
                if (previo) {
                    respuesta = `ℹ️ El soporte *ITEM ${r.soporte.item}* (${r.soporte.tipo}) ya está registrado como montado el ${formatearFechaChile(previo['Fecha'])} por ${previo['UsuarioReporte'] || '—'}. No se duplicó el registro.`;
                    action = 'SOPORTE_YA_MONTADO';
                } else {
                    const { fila } = await registrarMontajeSoporte({
                        soporte: r.soporte,
                        soldador: ia.soldador,
                        responsable: ia.responsable,
                        observacion: ia.observacion || (ia.hoja ? `Hoja ${ia.hoja}` : ''),
                        usuario: usuario.nombre || botConf.usuario_registro_defecto,
                        rol: usuario.rol
                    });
                    await guardarRegistro({
                        telefono, spool_tag: `ITEM ${r.soporte.item}`, id_spool: r.soporte.id,
                        status: 'Montado', observacion: ia.observacion,
                        appsheet_ok: true,
                        metadata: { entidad: 'soporte', id_montaje: fila['ID_MontajeSoporte'] }
                    });
                    respuesta = `✅ *Montaje de soporte registrado*\n\n🛠 Soporte: *ITEM ${r.soporte.item}* (${r.soporte.tipo})` +
                        `\n📏 Diámetro: ${r.soporte.diam || '—'}` +
                        `\n🧵 Línea: ${r.soporte.linea}` +
                        (ia.soldador ? `\n👨‍🏭 Soldador: ${ia.soldador}` : '') +
                        (ia.responsable ? `\n👷 Responsable: ${ia.responsable}` : '') +
                        (ia.observacion ? `\n📝 Obs: ${ia.observacion}` : '') +
                        `\n👤 ${usuario.nombre}`;
                    action = 'SOPORTE_REGISTRADO';
                }
            }

        } else if (ia.intencion === 'REGISTRAR_AVANCE' && entidad === 'spool' && itemDicho && ia.status) {
            ...
            if (!estadosPermitidos.includes(String(ia.status).toLowerCase())) {
                ...
                if (!r.encontrado) {
                    respuesta = `🔍 No encontré el spool "${itemDicho}" en el maestro del proyecto. Verifica el número e inténtalo de nuevo.`;
                    action = 'SPOOL_NO_ENCONTRADO';
                } else if (r.ambiguo) {
                    const lista = r.candidatos.map(c => `• ${c.tagGestion || c.idSpool}`).join('\n');
                    respuesta = `Encontré varios spools que coinciden con "${itemDicho}":\n${lista}\n\n¿Cuál corresponde?`;
        ...
        } else if (['CONSULTAR_ITEM', 'CONSULTAR_SPOOL'].includes(ia.intencion) && itemDicho) {
