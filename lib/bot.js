...
async function registrarAvanceAppSheet({ spool, status, usuario, ubicacion, observacion, mts, rol }) {
    // USUARIO tiene App Formula (usuario de sesión de la app) que la API no puede
    // llenar → el reportante va también en la observación para que quede visible.
    const obsConReporte = [observacion, `Reporta: ${usuario} (Andi)`].filter(Boolean).join(' — ');
    const fila = {
        'ID_LOG_SPOOL': idHex8(),
        ...
        'FOTO': '',
        'OBSERVACION TERRENO': obsConReporte,
...
async function registrarMontajeSoporte({ soporte, observacion, usuario, rol }) {
    // UsuarioReporte tiene App Formula (sesión de la app) → el reportante
    // también va en la observación para que quede visible en AppSheet.
    const obsConReporte = [observacion, `Reporta: ${usuario} (Andi)`].filter(Boolean).join(' — ');
    const fila = {
        ...
        'Fecha': fechaAppSheet(),
        'FotoTerreno': '',
        'ObservacionMontaje': obsConReporte,
        'UsuarioReporte': usuario
    };
    const userSettings = { "Rol": String(rol || 'TERRENO').toUpperCase().trim() };
...
async function handleWhatsappIncoming(req, res) {
    ...
    try {
        ...
        // 1. Autorización por número (con auto-autorización desde LIST_usuariosApp_MS)
        let usuario = await buscarUsuario(telefono);
        if (!usuario || !usuario.activo) {
            const auto = await autoAutorizarDesdeApp(telefono, pushName);
            if (auto) usuario = auto;
        }
        if (!usuario || !usuario.activo) {
