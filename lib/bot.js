...
/** Valida un nombre de soldador contra CAT_Personal_MS (la columna es restringida). */
async function resolverSoldador(nombre) {
    if (!nombre) return { valido: false, valor: null };
    ...
    return hit
        ? { valido: true, valor: String(hit['NOMBRES APELLIDOS']).trim() }
        : { valido: false, valor: String(nombre).trim() };
}
...
async function validarItems(itemsIA, usuario, botConf) {
    ...
    for (const it of (itemsIA || [])) {
        ...
        if (entidad === 'valvula') {
            ...
            // Soldador: columna restringida al catálogo de personal
            let soldador = null, obsExtra = '';
            if (it.soldador) {
                const s = await resolverSoldador(it.soldador);
                if (s.valido) soldador = s.valor;
                else obsExtra = `Soldador reportado: ${s.valor}`;
            }
            const observacion = [it.observacion, it.hoja ? `Hoja ${it.hoja}` : '', obsExtra].filter(Boolean).join(' | ') || null;
            validados.push({
                entidad: 'soporte', soporte: r.soporte, soldador, responsable: it.responsable || null, observacion,
                linea: `🛠 Soporte *ITEM ${r.soporte.item}* (${r.soporte.tipo}, línea ${r.soporte.linea}) → *Montado*` +
                    (soldador ? ` | Soldador: ${soldador}` : '') + (obsExtra ? ` | ⚠️ ${obsExtra} (no está en el catálogo, irá en observación)` : '')
            });
        ...
        try {
            if (v.entidad === 'valvula') {
                ...
                const { fila } = await registrarMontajeSoporte({
                    soporte: v.soporte, soldador: v.soldador, responsable: v.responsable, observacion: v.observacion,
                    usuario: usuario.nombre || botConf.usuario_registro_defecto, rol: usuario.rol
                });
                await guardarRegistro({
...
async function handleWhatsappIncoming(req, res) {
    ...
    try {
        ...
        if (ia.intencion === 'CONFIRMAR_REGISTRO' && pendiente) {
            const lineas = await ejecutarRegistros(pendiente.items, usuario, botConf, telefono);
            await borrarPendiente(telefono);
