...
async function registrarAvanceAppSheet({ spool, status, usuario, ubicacion, observacion, mts, rol, foto }) {
    const fila = {
        'ID_LOG_SPOOL': idHex8(),
        ...
        // FOTO es Drawing/Image requerido en AppSheet: si no viene foto, '-' evita el rechazo (400).
        'FOTO': (foto && String(foto).trim()) ? String(foto).trim() : '-',
...
async function ejecutarRegistros(validados, usuario, botConf, telefono) {
    ...
    for (const v of validados) {
        try {
            ...
            const errClean = e.message.replace(/^AppSheet Error \d+:\s*/i, '');
            lineas.push(`❌ ${v.linea.split('→')[0].trim()}: error al guardar (${errClean.substring(0, 100)})`);
        }
    }
    return lineas;
}
        ...
        if (usuario.invitado && ['REGISTRAR_AVANCE', 'CONFIRMAR_REGISTRO', 'SESION_ESCANEO'].includes(ia.intencion)) {
            ...
            if (entidadF === 'valvula') {
                ...
                if (r.encontrado && !r.ambiguo) {
                    ...
                    fotos = logs
                        .filter(x => String(x['ID_SPOOL'] || '').trim() === r.spool.idSpool && String(x['FOTO'] || '').trim() && x['FOTO'] !== '-' && x['FOTO'] !== 'N/A')
