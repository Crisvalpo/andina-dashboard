...
/** Filtra dinámicamente en tiempo real los acordeones de líneas, isométricos y tarjetas de spools */
export function bimFiltrarReemplazosUI(filtro) {
    const q = String(filtro || '').trim().toLowerCase();
    const lineaDetailsList = document.querySelectorAll('.bim-linea-accordion');
    let visiblesCount = 0;

    lineaDetailsList.forEach(lineaDet => {
        // Buscar en data-linea y en todo el texto visible del summary de la línea
        const summaryLineaText = lineaDet.querySelector('summary')?.textContent || '';
        const dataLinea = lineaDet.getAttribute('data-linea') || '';
        const lineaText = (dataLinea + ' ' + summaryLineaText).toLowerCase();
        const coincideLinea = q && lineaText.includes(q);

        const isoDetailsList = lineaDet.querySelectorAll('.bim-iso-accordion');
        let algunIsoCoincide = false;

        isoDetailsList.forEach(isoDet => {
            const summaryIsoText = isoDet.querySelector('summary')?.textContent || '';
            const dataIso = isoDet.getAttribute('data-iso') || '';
            const dataSheet = isoDet.getAttribute('data-sheet') || '';
            const isoText = (dataIso + ' ' + dataSheet + ' ' + summaryIsoText).toLowerCase();
            const coincideIso = q && (isoText.includes(q) || coincideLinea);

            const spoolCards = isoDet.querySelectorAll('.bim-reemplazo-spool-card');
            ...
            spoolCards.forEach(card => {
                ...
                const coincideSpool = !q || coincideLinea || coincideIso || spTag.includes(q) || spId.includes(q) || cardText.includes(q);
                card.style.display = coincideSpool ? '' : 'none';
                if (coincideSpool) algunSpoolCoincide = true;
            });

            if (!q) {
                isoDet.style.display = '';
                isoDet.open = false;
            } else if (coincideLinea || coincideIso || algunSpoolCoincide) {
                isoDet.style.display = '';
                isoDet.open = true; // Auto-abrir isométrico que coincide
                algunIsoCoincide = true;
            } else {
                isoDet.style.display = 'none';
            }
        });

        if (!q) {
            lineaDet.style.display = '';
            lineaDet.open = false; // Vuelven colapsadas al borrar el buscador
        } else if (coincideLinea || algunIsoCoincide) {
            lineaDet.style.display = '';
            lineaDet.open = true; // Auto-abrir línea encontrada
            visiblesCount++;
        } else {
            lineaDet.style.display = 'none';
        }
    });
...
/** Expande o colapsa todos los acordeones de líneas e isométricos */
export function bimToggleTodasLineas(expandir) {
    document.querySelectorAll('.bim-linea-accordion').forEach(det => {
        det.open = !!expandir;
    });
    document.querySelectorAll('.bim-iso-accordion').forEach(det => {
        det.open = !!expandir;
    });
}

/** Renderiza la lista completa interactiva organizada por Línea -> Isométrico (Hoja) -> Spool. */
export async function bimRenderReemplazosList(forzar = false) {
    // Si ya existe en caché de cliente y no se solicita forzar, pintar inmediatamente en 0ms
    if (!forzar && bimState.reemplazoLineasCache) {
        bimPintarReemplazosHtml(bimState.reemplazoLineasCache);
        return;
    }

    bimSetMetaCargando('Cargando reemplazos por línea e isométrico...');
    try {
        const res = await fetch('/api/bim/reemplazo/lineas');
        const data = await res.json();

        if (data && data.success) {
            bimState.reemplazoLineasCache = data;
        }
        bimPintarReemplazosHtml(data);
    } catch (e) {
...
/** Función auxiliar que genera el HTML de la jerarquía de reemplazos */
function bimPintarReemplazosHtml(data) {
    ...
    const accordionsHtml = data.lineas.map((l, lIdx) => {
        const attrLinea = (l.linea || 'Sin Línea').replace(/"/g, '&quot;');

        // Subgrupos de Isométricos (vienen colapsados sin atributo open)
        const isosHtml = (l.isos || []).map((iso, isoIdx) => {
                ...
                <div class="bim-reemplazo-spool-card" data-spool="${sp.tag}" data-idspool="${(d.idSpool || rawIdSpool).replace(/"/g, '&quot;')}" style="background: rgba(236,72,153,0.06); border: 1px solid rgba(236,72,153,0.22); border-left: 3px solid #ec4899; border-radius: 5px; padding: 7px 8px; margin-bottom: 6px;">
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        ...
                        <div style="display:flex; align-items:center; gap:4px;">
                            ${sp.fotosCount > 0 ? `<span class="bim-badge" title="Tiene fotos registradas" style="background:rgba(16,185,129,0.2); color:#6ee7b7; border:1px solid rgba(16,185,129,0.35); font-size:0.63rem; padding:1px 5px;"><i class="fas fa-camera"></i> Foto</span>` : ''}
                    ...
                    <div style="display:flex; gap:5px; margin-top:6px;">
                        ...
                        <button onclick="bimBuscarReemplazoPorIndice(${lIdx}, ${isoIdx}, ${spIdx})" class="bim-scan-btn" style="padding:3px 6px; font-size:0.69rem; background:rgba(99,102,241,0.2); border-color:rgba(99,102,241,0.4); color:var(--primary-light); flex:1; justify-content:center;">
            ...
            return `
            <details class="bim-iso-accordion" data-iso="${attrIso}" data-sheet="${iso.sheet || ''}" style="margin-bottom: 7px; background: rgba(0,0,0,0.25); border: 1px solid rgba(56,189,248,0.28); border-radius: 6px; overflow: hidden;">
                <summary style="display: flex; align-items: center; justify-content: space-between; padding: 6px 9px; background: rgba(56,189,248,0.12); cursor: pointer; gap: 8px;">
                    <div style="display: flex; align-items: center; gap: 6px; flex-shrink: 0;">
                        <i class="fas fa-chevron-right bim-sub-chevron" style="font-size:0.62rem; color:#38bdf8; transition: transform 0.2s;"></i>
                        <i class="fas fa-file-lines" style="font-size:0.75rem; color:#38bdf8;"></i>
                        <strong style="color: #38bdf8; font-size: 0.8rem; letter-spacing: 0.5px; font-family: monospace; white-space: nowrap;">
                            ${iso.label || ('HOJA ' + (iso.sheet || '1'))}
                        </strong>
                        <span style="font-size: 0.65rem; color: #94a3b8; white-space: nowrap;">
                            (${iso.totalSpools} spools)
                        </span>
                    </div>
                    <button onclick="event.stopPropagation(); bimAislarIsoPorIndice(${lIdx}, ${isoIdx})" title="Aislar y enfocar isométrico completo en 3D" class="bim-scan-btn" style="padding: 2px 7px; font-size: 0.65rem; background: rgba(56,189,248,0.22); border: 1px solid rgba(56,189,248,0.45); color: #bae6fd; flex-shrink: 0; white-space: nowrap; display: flex; align-items: center; gap: 3px;">
                        <i class="fas fa-eye"></i> ISO 3D
                    </button>
                </summary>
        ...
        // Líneas principales (vienen colapsadas sin atributo open)
        return `
        <details class="bim-linea-accordion" data-linea="${attrLinea}" style="margin-bottom: 10px; background: rgba(255,255,255,0.02); border: 1px solid rgba(236,72,153,0.3); border-radius: 8px; overflow: hidden;">
            <summary style="display: flex; flex-direction: column; padding: 9px 10px; background: linear-gradient(135deg, rgba(236,72,153,0.14), rgba(30,27,75,0.35)); cursor: pointer; gap: 6px;">
                <!-- Fila 1: Nombre completo de la línea -->
                <div style="display: flex; align-items: flex-start; gap: 6px; width: 100%;">
                    <i class="fas fa-chevron-right bim-linea-chevron" style="font-size:0.68rem; color:#f472b6; margin-top:3px; transition: transform 0.2s; flex-shrink:0;"></i>
                    <i class="fas fa-grip-lines" style="font-size:0.74rem; color:#f472b6; margin-top:3px; flex-shrink:0;"></i>
                    <span title="${attrLinea}" style="font-weight: 700; color: #fbcfe8; font-size: 0.8rem; font-family: monospace; word-break: break-all; line-height: 1.25; flex: 1;">
                        ${l.linea}
                    </span>
                </div>
                <!-- Fila 2: Chips horizontales alineados (sin wrap) + Botón Línea 3D -->
                <div style="display: flex; align-items: center; justify-content: space-between; width: 100%; padding-left: 14px; box-sizing: border-box; gap: 6px;">
                    <div style="display: flex; align-items: center; gap: 4px; overflow: hidden; white-space: nowrap;">
                        <span style="display: inline-flex; align-items: center; gap: 3px; background: rgba(56,189,248,0.18); color: #7dd3fc; border: 1px solid rgba(56,189,248,0.35); font-size: 0.62rem; padding: 1px 6px; border-radius: 10px; font-weight: 600; white-space: nowrap; flex-shrink: 0;">
                            <i class="fas fa-layer-group" style="font-size:0.56rem;"></i> ${l.totalIsos} hojas
                        </span>
                        <span style="display: inline-flex; align-items: center; gap: 3px; background: rgba(236,72,153,0.2); color: #fbcfe8; border: 1px solid rgba(236,72,153,0.35); font-size: 0.62rem; padding: 1px 6px; border-radius: 10px; font-weight: 600; white-space: nowrap; flex-shrink: 0;">
                            <i class="fas fa-arrows-rotate" style="font-size:0.56rem;"></i> ${l.totalSpools} spools
                        </span>
                        <span style="display: inline-flex; align-items: center; background: rgba(255,255,255,0.07); color: var(--text-dim); font-size: 0.62rem; padding: 1px 5px; border-radius: 10px; white-space: nowrap; flex-shrink: 0;">
                            ${l.totalElementos} el.
                        </span>
                    </div>
                    <button onclick="event.stopPropagation(); bimAislarLineaPorIndice(${lIdx})" 
                            title="Aislar línea completa: Spools normales en amarillo y a reemplazar en rosa" 
                            class="bim-scan-btn" 
                            style="padding: 2px 8px; font-size: 0.67rem; background: rgba(236,72,153,0.28); border: 1px solid rgba(236,72,153,0.55); color: #fff; border-radius: 5px; white-space: nowrap; flex-shrink: 0; display: flex; align-items: center; gap: 4px;">
    ...
    bimSetMeta(`
        <style>
            .bim-linea-accordion > summary, .bim-iso-accordion > summary { list-style: none; user-select: none; }
            .bim-linea-accordion > summary::-webkit-details-marker, .bim-iso-accordion > summary::-webkit-details-marker { display: none; }
            .bim-linea-accordion[open] > summary .bim-linea-chevron { transform: rotate(90deg); }
            .bim-iso-accordion[open] > summary .bim-sub-chevron { transform: rotate(90deg); }
        </style>
        <div class="bim-meta-header" style="background: rgba(236,72,153,0.18); border-color: rgba(236,72,153,0.35);">
            ...
            <span style="font-weight:700;">Reemplazos por Línea e ISO</span>
        ...
        <div style="padding: 8px 0 4px 0;">
            <div style="position: relative; margin-bottom: 6px;">
                <input type="text" id="bim-reemplazo-busqueda" placeholder="Buscar línea, isométrico o spool..." 
            ...
            <!-- Leyenda de colores para Línea 3D -->
            <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom: 6px; padding: 3px 7px; background: rgba(0,0,0,0.28); border: 1px solid rgba(255,255,255,0.06); border-radius: 5px; font-size: 0.65rem;">
                <span style="color:var(--text-dim); font-weight:600;"><i class="fas fa-palette" style="margin-right:3px;"></i>Línea 3D:</span>
                <span style="display:inline-flex; align-items:center; gap:3px; color:#fde047;">
                    <span style="width:8px; height:8px; border-radius:2px; background:#facc15; display:inline-block;"></span> Spools
                </span>
                <span style="display:inline-flex; align-items:center; gap:3px; color:#f472b6;">
                    <span style="width:8px; height:8px; border-radius:2px; background:#ec4899; display:inline-block;"></span> Reemplazo
                </span>
            </div>
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 8px; font-size: 0.7rem; color: var(--text-dim);">
                <span>${data.totalLineas} líneas • ${data.totalIsos} hojas • ${data.totalSpools} spools</span>
...
/**
 * Aísla en el visor 3D la línea de cañería completa:
 * - Colorea en AMARILLO (#facc15) todos los spools normales de la línea.
 * - Colorea en ROSA (#ec4899) los spools asignados para reemplazo.
 * Se ejecuta de forma instantánea sin peticiones de red ni refresco del panel lateral.
 */
export async function bimAislarLineaConReemplazos(lineaId, guidsReemplazo = [], guidsTotalesPre = [], guidsNormalesPre = []) {
    if (!bimState.viewer) return;
    const viewer = bimState.viewer;

    try {
        let todosGuids = Array.isArray(guidsTotalesPre) && guidsTotalesPre.length ? [...guidsTotalesPre] : [];

        // Si no vienen precomputados, obtener de la API de respaldo
        if (!todosGuids.length) {
            const resp = await fetch(`/api/bim/linea/item/${encodeURIComponent(lineaId)}`);
            const data = resp.ok ? await resp.json() : null;
            if (data && Array.isArray(data.guids)) {
                todosGuids = data.guids;
            }
        }

        const setReemplazo = new Set(
            (Array.isArray(guidsReemplazo) ? guidsReemplazo : [guidsReemplazo])
                .map(g => String(g).trim().toLowerCase())
                .filter(Boolean)
        );

        // Asegurar que los de reemplazo estén en el conjunto total
        setReemplazo.forEach(g => {
            const baseG = g.split('#p')[0];
            if (!todosGuids.some(tg => tg.toLowerCase() === baseG.toLowerCase())) {
                todosGuids.push(baseG);
            }
        });

        // Separar guids de elementos reales (sin '#p') para resolución en Forge Viewer
        const guidsParaForge = [...new Set(todosGuids.map(g => g.split('#p')[0]).filter(Boolean))];

        if (!guidsParaForge.length) {
            alert(`No se encontraron elementos 3D para la línea "${lineaId}".`);
            return;
        }

        // Limpiar coloraciones previas y selección para evitar que el azul de selección oculte el theming
        viewer.clearThemingColors(viewer.model);
        viewer.clearSelection();

        // Convertir a dbIds de Forge
        bimGuidsToDbIds(guidsParaForge, (dbIds) => {
            if (!dbIds || !dbIds.length) {
                console.warn('[Línea 3D] No se pudieron mapear dbIds para:', guidsParaForge);
                return;
            }

            // Mapear cada dbId a su externalId/GUID para discriminar Reemplazo vs Normal
            viewer.model.getBulkProperties(dbIds, { propFilter: ['externalId', 'GUID', 'Element GUID'] }, (results) => {
                const dbIdsReemplazo = [];
                const dbIdsNormales = [];

                (results || []).forEach(r => {
                    const extId = String(r.externalId || '').trim().toLowerCase();
                    if (setReemplazo.has(extId)) {
                        dbIdsReemplazo.push(r.dbId);
                    } else {
                        dbIdsNormales.push(r.dbId);
                    }
                });

                if (!dbIdsReemplazo.length && setReemplazo.size) {
                    const baseReemp = Array.from(setReemplazo).map(g => g.split('#p')[0]);
                    bimGuidsToDbIds(baseReemp, (rIds) => {
                        aplicarVisualizacion3D(rIds, dbIds.filter(id => !rIds.includes(id)));
                    });
                } else {
                    aplicarVisualizacion3D(dbIdsReemplazo, dbIdsNormales);
                }
            });

            function aplicarVisualizacion3D(reempIds, normalIds) {
                const todosDbIds = [...new Set([...reempIds, ...normalIds])].filter(id => !divState.ocultos.includes(id));
                bimState.dbIds = todosDbIds;
                viewer.clearSelection();
                viewer.isolate(todosDbIds);
                viewer.fitToView(todosDbIds);

                if (typeof bimDivReocultarOriginales === 'function') bimDivReocultarOriginales();

                // 🟡 AMARILLO para spools normales de la línea (#facc15)
                const colAmarillo = new THREE.Vector4(0.98, 0.80, 0.08, 0.88);
                normalIds.forEach(id => viewer.setThemingColor(id, colAmarillo, viewer.model, true));

                // 🌸 ROSA para spools a reemplazar (#ec4899)
                const colRosa = new THREE.Vector4(0.925, 0.282, 0.600, 0.92);
                reempIds.forEach(id => viewer.setThemingColor(id, colRosa, viewer.model, true));

                // Colorear trozos divididos (si existen)
                if (divState.trozoMeshes) {
                    Object.entries(divState.trozoMeshes).forEach(([k, mesh]) => {
                        const kLower = k.toLowerCase();
                        const baseG = kLower.split('#p')[0];
                        if (guidsParaForge.some(g => g.toLowerCase() === baseG)) {
                            mesh.visible = true;
                            if (setReemplazo.has(kLower) || setReemplazo.has(baseG)) {
                                mesh.material.color.setHex(0xec4899); // Rosa
                            } else {
                                mesh.material.color.setHex(0xfacc15); // Amarillo
                            }
        ...
        console.error('[BIM Aislar Línea con Reemplazos]', err);
    }
}

/** Aísla la línea usando su índice en la caché (100% inmune a nombres con comillas dobles o caracteres especiales) */
export function bimAislarLineaPorIndice(lineaIdx) {
    const lineas = bimState.reemplazoLineasCache?.lineas;
    if (!lineas || !lineas[lineaIdx]) return;
    const l = lineas[lineaIdx];
    bimAislarLineaConReemplazos(l.linea, l.guids, l.guidsLineaTotales, l.guidsLineaNormales);
}

/** Aísla el isométrico/hoja por índice */
export function bimAislarIsoPorIndice(lineaIdx, isoIdx) {
    const iso = bimState.reemplazoLineasCache?.lineas?.[lineaIdx]?.isos?.[isoIdx];
    if (!iso || !iso.guids) return;
    bimFocoElementoRedline(iso.guids);
}

/** Enfoca el spool en 3D por índice */
export function bimFocoSpoolPorIndice(lineaIdx, isoIdx, spIdx) {
    const sp = bimState.reemplazoLineasCache?.lineas?.[lineaIdx]?.isos?.[isoIdx]?.spools?.[spIdx];
    if (!sp || !sp.guids) return;
    bimFocoElementoRedline(sp.guids);
}

/** Abre la ficha de fotos y redline del spool por índice */
export function bimBuscarReemplazoPorIndice(lineaIdx, isoIdx, spIdx) {
    const l = bimState.reemplazoLineasCache?.lineas?.[lineaIdx];
    const iso = l?.isos?.[isoIdx];
    const sp = iso?.spools?.[spIdx];
    if (!sp) return;
    bimBuscarReemplazoTag(sp.tag, l.linea, iso.idIso);
}

/** Despliega la vista detallada de un spool a reemplazar con su ficha de fotos y referencias. */
export async function bimBuscarReemplazoTag(tag, lineaNombre = '', isoNombre = '') {
    if (!tag) return;
...
export function bimTrozoPointerUp(ev) {
    ...
    // Los trozos se pueden vincular en capa spool, subsistema o reemplazo.
    // En válvulas/soportes no tiene sentido → dejar pasar el clic al visor.
    if (bimState.capa !== 'spool' && bimState.capa !== 'subsistema' && bimState.capa !== 'reemplazo') { divState._downSel = null; return; }
...
/** Panel del trozo: spool/subsistema/reemplazo asignado, estado y asignación/desvinculación. */
export function bimTrozoRenderPanel(mesh) {
    ...
    const esReemplazo = bimState.capa === 'reemplazo';
    ...
    let status = null;
    if (esReemplazo) {
        status = reemplazoAsignado ? 'REEMPLAZO' : null;
    } else if (bimState.statusesCache) {
    ...
    const vinculoActivo = esSub ? subAsignado : (esReemplazo ? reemplazoAsignado : tagAsignado);
    const vinculoLabel = esSub ? 'Sub-sistema' : (esReemplazo ? 'Spool a Reemplazar' : 'Spool');
    const vinculoColor = esSub ? '#c4b5fd' : (esReemplazo ? '#f472b6' : '#6ee7b7');
    const vinculoBg = esSub ? 'rgba(139,92,246,0.08)' : (esReemplazo ? 'rgba(236,72,153,0.08)' : 'rgba(16,185,129,0.08)');
    const vinculoBorder = esSub ? 'rgba(139,92,246,0.25)' : (esReemplazo ? 'rgba(236,72,153,0.25)' : 'rgba(16,185,129,0.25)');

    if (esSub) bimPopulateDatalist('subsistema');

    bimSetMeta(`
        <div class="bim-meta-header" style="background:${esReemplazo ? 'rgba(236,72,153,0.2)' : 'rgba(96,165,250,0.15)'};border-color:${esReemplazo ? 'rgba(236,72,153,0.4)' : 'rgba(96,165,250,0.35)'};">
            <i class="fas fa-puzzle-piece" style="${esReemplazo ? 'color:#f472b6;' : ''}"></i><span>Trozo ${idx + 1}</span>
            <span class="bim-badge" style="${esReemplazo ? 'background:#ec4899;' : ''}">${pct}% del tramo</span>
        </div>
        ${vinculoActivo ? `
        ...
        <div class="bim-link-field" style="margin-bottom:8px;">
            <label style="font-size:0.75rem;opacity:0.8;">${esSub ? 'Sub-sistema para este trozo:' : (esReemplazo ? 'TAG del spool a reemplazar para este trozo:' : 'TAG del spool para este trozo:')}</label>
            <input type="text" id="trozo-spool-input" class="bim-search-input" placeholder="${esSub ? 'Ej: 03350-02-06' : (esReemplazo ? 'Ej: 412' : 'Ej: 511')}" value="${vinculoActivo || ''}" style="width:100%;margin-top:4px;"${esSub ? ' list="bim-link-datalist"' : ''}>
        </div>
        <button id="trozo-vincular-btn" class="bim-scan-btn" onclick="bimTrozoVincular('${key}')" style="background:${esReemplazo ? 'rgba(236,72,153,0.2)' : 'rgba(99,102,241,0.15)'};border-color:${esReemplazo ? 'rgba(236,72,153,0.4)' : 'rgba(99,102,241,0.3)'};color:${esReemplazo ? '#f472b6' : 'var(--primary-light)'};justify-content:center;width:100%;">
...
export async function bimTrozoVincular(key) {
    const esSub = bimState.capa === 'subsistema';
    const esReemplazo = bimState.capa === 'reemplazo';
    const labelCapa = esSub ? 'sub-sistema' : (esReemplazo ? 'reemplazo' : 'spool');
    console.log(`[Trozo] Vincular ${labelCapa} solicitado:`, key);

    // Solo se permite vincular trozos en capa spool, subsistema o reemplazo
    if (bimState.capa !== 'spool' && bimState.capa !== 'subsistema' && bimState.capa !== 'reemplazo') {
        alert('Los trozos solo se asignan a spools, sub-sistemas o reemplazos.');
        return;
    }
    const mesh = divState.trozoMeshes[key];
    ...
    if (!tag) { alert(`Ingresa el ${esSub ? 'código del sub-sistema' : (esReemplazo ? 'TAG del spool a reemplazar' : 'TAG del spool')}.`); if (input) input.focus(); return; }
    ...
    try {
        ...
        if (esSub) {
            ...
            // Reemplazo: POST /api/bim/reemplazo/vincular
            resp = await fetch('/api/bim/reemplazo/vincular', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', ...authHeaders('bim') },
                body: JSON.stringify({ item: targetItem, elements: [{ guid: key, cwp: '', line_number: '', tag: '', autocad_size: '' }] })
            });
        ...
        if (esSub) {
            ...
            if (!bimState.capaMapeo['reemplazo']) bimState.capaMapeo['reemplazo'] = {};
            bimState.capaMapeo['reemplazo'][key.toLowerCase()] = targetItem;
            if (!bimState.capaStatuses) bimState.capaStatuses = { 'REEMPLAZO': [] };
            if (!bimState.capaStatuses['REEMPLAZO']) bimState.capaStatuses['REEMPLAZO'] = [];
            if (!bimState.capaStatuses['REEMPLAZO'].some(g => String(g).toLowerCase() === key.toLowerCase())) {
                bimState.capaStatuses['REEMPLAZO'].push(key);
            }
        } else {
...
export async function bimTrozoDesvincular(key) {
    ...
    try {
        ...
        if (esSub) {
            ...
            // Desvincular reemplazo: usa endpoint genérico de capa
            resp = await fetch('/api/bim/reemplazo/desvincular', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', ...authHeaders('bim') },
                body: JSON.stringify({ elements: [{ guid: key }] })
            });
        ...
        if (esSub) {
            ...
            if (bimState.capaMapeo['reemplazo']) delete bimState.capaMapeo['reemplazo'][key.toLowerCase()];
            if (bimState.capaStatuses && bimState.capaStatuses['REEMPLAZO']) {
                const i = bimState.capaStatuses['REEMPLAZO'].findIndex(g => String(g).toLowerCase() === key.toLowerCase());
...
export function bimStatusPorGuid() {
    const m = {};
    const statuses = (bimState.capa === 'spool' || !bimState.capaStatuses) ? bimState.statusesCache : bimState.capaStatuses;
    if (statuses) {
        for (const [st, gs] of Object.entries(statuses)) {
...
export function bimDivFiltrarTrozos(seleccionSet) {
    const statusDe = bimStatusPorGuid();
    const subMapeo = bimState.capaMapeo['subsistema'] || {};
    const rMapeo = bimState.capaMapeo['reemplazo'] || {};
    ...
    for (const [key, mesh] of Object.entries(divState.trozoMeshes)) {
        ...
        if (seleccionSet) {
            ...
            } else if (bimState.capa === 'reemplazo') {
                const rTag = rMapeo[keyLower] || rMapeo[parentGuid];
                mesh.visible = seleccionSet.has('REEMPLAZO') ? !!rTag : false;
            } else {
...
export function bimFocoElementoRedline(guids) {
    ...
    // Separar elementos reales de trozos (guid#pN)
    const guidsParaAislar = [];
    list.forEach(g => {
        if (g.includes('#p')) {
            const mesh = divState.trozoMeshes ? divState.trozoMeshes[g] : null;
            if (mesh?.userData?.guid) {
                guidsParaAislar.push(mesh.userData.guid);
            } else {
                const baseGuid = g.split('#p')[0];
                if (baseGuid) guidsParaAislar.push(baseGuid);
            }
        } else {
            guidsParaAislar.push(g);
        }
    ...
    bimGuidsToDbIds(guidsParaAislar.length ? guidsParaAislar : list, (dbIds) => {
    ...
    // Trozos: mantener sólidos los trozos de la lista y el resto en x-ray/ocultos si aplica
    if (typeof bimDivGhostPorSpool === 'function') {
        bimDivGhostPorSpool(list);
    }
}
...
if (typeof window !== 'undefined') {
    ...
    window.bimAislarLineaConReemplazos = bimAislarLineaConReemplazos;
    window.bimAislarLineaPorIndice = bimAislarLineaPorIndice;
    window.bimAislarIsoPorIndice   = bimAislarIsoPorIndice;
    window.bimFocoSpoolPorIndice   = bimFocoSpoolPorIndice;
    window.bimBuscarReemplazoPorIndice = bimBuscarReemplazoPorIndice;
